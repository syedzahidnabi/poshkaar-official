...
export default function AtelierSignature() {
  return (
    <section className="relative overflow-hidden bg-ivory py-20 md:py-32" aria-labelledby="atelier-signature-title">
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 md:px-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:px-16">
        <motion.div
        ...
        >
          <div className="mb-7 inline-flex items-center gap-3 border border-gold/25 bg-white/45 px-4 py-2 text-[9px] uppercase tracking-[0.24em] text-walnut backdrop-blur">
          ...
          <Link
            to="/our-story"
            className="group mt-9 inline-flex items-center gap-3 text-[10px] uppercase tracking-[0.24em] text-walnut luxury-transition hover:text-gold"
          >
            Read the story
        ...
        <div className="grid gap-4">
          {DETAILS.map((detail, index) => (
            <motion.article
              key={detail.title}
              className="group relative overflow-hidden border border-walnut/10 bg-sand/70 p-6 shadow-[0_24px_80px_-60px_rgba(91,58,41,0.8)] backdrop-blur luxury-transition hover:-translate-y-1 hover:border-gold/35 hover:bg-white/55 md:p-7"
            ...
            >
              ...
              <div className="flex gap-5">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center border border-gold/25 bg-ivory text-gold">
                  <detail.icon size={19} strokeWidth={1.4} aria-hidden="true" />
                </div>
                <div>
                  <h3 className="font-heading text-2xl font-light text-charcoal">{detail.title}</h3>
                  <p className="mt-3 text-sm leading-7 text-charcoal/64">{detail.text}</p>
                </div>
              </div>
            </motion.article>
          ))}
