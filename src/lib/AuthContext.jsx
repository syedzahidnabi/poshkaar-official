...
export const AuthProvider = ({ children }) => {
  ...
  const checkAppState = async () => {
    try {
      ...
      if (backendProvider === 'supabase') {
        ...
        await checkUserAuth({ silentNoSession: true });
        setIsLoadingPublicSettings(false);
        return;
      }
  ...
  const checkUserAuth = async ({ silentNoSession = false } = {}) => {
    try {
      ...
      if (!silentNoSession && (error.status === 401 || error.status === 403)) {
        setAuthError({
          type: 'auth_required',
          message: 'Authentication required'
        });
  ...
  const navigateToLogin = () => {
    const returnTo = getSafeAuthRedirect(window.location.href);
    window.location.assign(withFromUrl('/login', returnTo));
  };
